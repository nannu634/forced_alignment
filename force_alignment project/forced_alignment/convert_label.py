...
def read_lab(filename):
    ...
    with open(filename, 'r', encoding='utf-8') as f:  # Specify the encoding
        text = f.read()
        segments_text = re.findall(r'<s>(.*?)<\/s>', text)
        segments = []

        for i, text_segment in enumerate(segments_text):
            tStart = i  # Use the index as a placeholder for tStart
            tEnd = i + 1  # Use the index as a placeholder for tEnd
            label = text_segment.strip()
